...
## 4. Saran Stack Cepat untuk Hackathon
- **Backend**: Express/NestJS + PostgreSQL + Prisma (cocok dengan pengalamanmu di OpenJob)
- **Auth**: JWT untuk mahasiswa/DPL/admin/kaprodi login; token acak (JWT sekali pakai atau UUID + expiry) untuk link email DPL/Mitra
- **Email**: Nodemailer + queue (RabbitMQ, seperti di OpenJob v2) supaya kirim email penilaian tidak blocking
- **File upload**: Multer, simpan path di tabel `dokumen`
- **Export Excel**: exceljs / xlsx
- **Frontend**: React + Vite, dashboard pakai Recharts untuk statistik Kaprodi
