...
## 8. DETAIL FLOW REST API — ROLE MAHASISWA

Semua request (kecuali auth) wajib header:
```
Authorization: Bearer {sanctum_token}
Accept: application/json
```

### 8.1 Login (Google OAuth)
```
...
GET  /api/auth/google/callback?code=...
     → response 200:
     {
       "user": { "id": 1, "nama": "...", "email": "...", "role": "mahasiswa" },
       "token": "1|xxxxxxxxxxxx"
     }
```
Frontend simpan token, dipakai di semua request berikutnya.

### 8.2 Tahap 1 — Pengajuan Magang
```
POST /api/magang
Body (multipart/form-data):
  mitra_industri_id, supervisor_mitra_id (atau data mitra baru jika belum ada),
  dpl_id, posisi, periode_mulai, periode_selesai,
  proposal_file (file), bukti_diterima_file (file)
...
```
GET  /api/magang/{id}
→ 200: detail data magang + status terkini (draft/menunggu_verifikasi/disetujui/ditolak)

GET  /api/magang            (list magang milik mahasiswa yg login)
→ 200: [ {...}, {...} ]
```
> Kalau status masih `menunggu_verifikasi` atau `ditolak`, tombol lanjut ke Usulan Konversi di frontend **disable**.

### 8.3 Tahap 2 — Usulan Konversi
Hanya bisa diakses kalau `magang.status == disetujui`.
```
GET  /api/mata-kuliah
→ 200: daftar MK (dari SikurOBE/master data) untuk dropdown pilihan

GET  /api/mata-kuliah/{id}/cpmk
→ 200: daftar CPMK milik MK tsb, untuk dropdown pilihan
...
Body:
{
  "magang_id": 10,
  "details": [
    { "mata_kuliah_id": 3, "cpmk_id": 12, "deskripsi_aktivitas_rencana": "Membangun REST API" },
    { "mata_kuliah_id": 5, "cpmk_id": 20, "deskripsi_aktivitas_rencana": "..." }
  ]
}
→ 201 Created
{ "id": 4, "status": "menunggu_persetujuan_dpl" }
```
```
GET  /api/usulan-konversi/{id}
→ 200: detail usulan + status (menunggu_persetujuan_dpl/disetujui/revisi/ditolak) + catatan_dpl

PUT  /api/usulan-konversi/{id}   (khusus kalau status = revisi)
Body: sama seperti POST, mahasiswa perbaiki lalu submit ulang
→ 200: status kembali ke menunggu_persetujuan_dpl
```

### 8.4 Tahap 3 — Klaim Konversi
Hanya bisa diakses kalau `usulan_konversi.status == disetujui` DAN periode magang sudah selesai.
```
POST /api/klaim-konversi
Body (multipart/form-data):
{
  "usulan_konversi_id": 4,
  "logbook_file": file,
  "laporan_file": file,
  "sertifikat_file": file,
  "dokumen_lain_file": file (opsional),
  "details": [
    { "usulan_konversi_detail_id": 8, "bukti_aktivitas_text": "Pengembangan REST API dan deployment aplikasi", "bukti_file": file (opsional) },
    ...
  ]
}
→ 201 Created
{ "id": 6, "status": "menunggu_penilaian_mitra" }
```
> Setelah submit, backend otomatis: generate signed URL untuk mitra → kirim email (job di-queue).
...
{
  "id": 6,
  "status": "menunggu_review_dpl",   // atau menunggu_penilaian_mitra/revisi/disetujui/ditolak
  "penilaian_mitra": { "nilai": 90, "komentar": "..." } | null,
  "penilaian_dpl": null,
  "nilai_akhir": null
}
```
```
GET  /api/klaim-konversi/{id}/hasil   (setelah status = disetujui & nilai_akhir sudah dihitung)
→ 200:
{
  "mata_kuliah": [
    { "kode_mk": "IF301", "nama_mk": "Pemrograman Web", "sks": 3, "nilai_akhir": 88.5, "nilai_huruf": "A" }
  ]
}
```

### 8.6 Ringkasan sequence (happy path)
```
Login Google
  → POST /magang
  → (tunggu admin verifikasi) GET /magang/{id} → status: disetujui
  → GET /mata-kuliah, GET /mata-kuliah/{id}/cpmk
  → POST /usulan-konversi
  → (tunggu DPL) GET /usulan-konversi/{id} → status: disetujui
  → POST /klaim-konversi
  → (tunggu mitra & DPL, mahasiswa tinggal poll/cek status)
  → GET /klaim-konversi/{id} → status: disetujui
  → GET /klaim-konversi/{id}/hasil → tampilkan nilai akhir & SKS
```

### 8.7 Error handling yang perlu ditangani di frontend
| Kondisi | HTTP Status | Penanganan |
|---|---|---|
| Ajukan usulan sebelum magang disetujui | 422 | Tombol disable di UI + pesan jelas |
| Ajukan klaim sebelum usulan disetujui | 422 | Tombol disable, arahkan ke status usulan |
| Ajukan usulan/klaim dobel (masih pending) | 409 | Tampilkan status yang sedang berjalan |
| Upload file > batas ukuran/tipe salah | 422 | Validasi juga di frontend sebelum submit |
| Token Sanctum expired | 401 | Redirect ke login Google ulang |
