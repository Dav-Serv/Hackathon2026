...
function App() {
  ...
  const navLinkClass = (name) => `rounded-full px-2.5 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-xs font-bold transition-all ${activeNav === name ? 'border-2 border-[#191b23] bg-[#9f149f] text-white shadow-[2px_2px_0_#191b23] sm:shadow-[3px_3px_0_#191b23]' : 'hover:bg-[#e7e7f3]'}`

  const stageClass = (stage) => `hidden lg:absolute z-20 cursor-pointer opacity-60 transition duration-300 hover:z-50 hover:scale-110 hover:opacity-100 ${selectedStage === stage ? 'z-50 !opacity-100 !border-[#9f149f] shadow-[0_0_20px_rgba(159,20,159,.4),12px_12px_0_#191b23]' : ''}`
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#faf8ff] font-['Space_Grotesk',sans-serif] text-[#191b23] [background-image:radial-gradient(#e1e2ed_1px,transparent_1px)] [background-size:24px_24px]">
      ...
      <main>
        <section id="beranda" className="journey-map-bg relative flex min-h-[600px] lg:min-h-[860px] items-center justify-center overflow-hidden border-b-4 border-[#191b23] px-5 py-24 sm:py-32 lg:py-36 md:px-16">
          <div className="absolute inset-0 z-0 opacity-70" aria-hidden="true">
            <CursorGrid cellSize={70} color="#a862a8" radius={140} falloff="smooth" holdTime={400} fadeDuration={800} lineWidth={1.2} maxOpacity={1} fillOpacity={0} gridOpacity={0} cellRadius={0} clickPulse={false} pulseSpeed={600} />
          </div>
          <div className="pointer-events-none absolute inset-0 opacity-10 hidden lg:block"><svg className="h-full w-full" viewBox="0 0 1440 900" fill="none"><path d="M250 220Q400 200 550 280M850 250Q1050 280 1150 450M1150 600Q1100 750 850 780M550 780Q350 780 250 650" stroke="#191b23" strokeDasharray="8 8" strokeWidth="3" /></svg></div>
          ...
          <div className="relative z-40 flex w-full max-w-4xl flex-col items-center text-center">
            <span className="mb-8 rotate-[-1deg] rounded-full border-[3px] border-[#191b23] bg-[#64a8fe] px-5 py-2 text-xs font-bold uppercase tracking-[.18em] shadow-[4px_4px_0_#191b23]">OBE STANDARD 2.0 READY</span>
            <h1 className="text-3xl sm:text-5xl lg:text-[76px] font-bold uppercase leading-[1.1] lg:leading-[.92] tracking-tighter">
              Permudah Konversi<br />
              <span className="my-2 inline-block rotate-1 bg-[#f1d7f1] px-4 py-1 text-[#9f149f]">Nilai Magang</span><br />
              Berbasis OBE.
            </h1>
            <p className="mt-8 max-w-xl border-l-4 border-[#9f149f] bg-white/60 p-4 text-sm sm:text-base lg:text-lg leading-relaxed text-[#434655] backdrop-blur-sm">
              Platform eksklusif Universitas AMIKOM Yogyakarta untuk efisiensi pengajuan, verifikasi, dan otomasi pemetaan mata kuliah pilihan secara visual.
            </p>
            <div className="relative mt-10 flex flex-col gap-5 rounded-[20px] sm:rounded-[28px] border-4 border-[#191b23] bg-[#ededf9] p-5 sm:p-7 shadow-[8px_8px_0_#191b23] sm:shadow-[12px_12px_0_#191b23] sm:flex-row">
              <span className="absolute -top-5 left-7 border-[3px] border-[#191b23] bg-[#9f149f] px-4 py-1 text-xs font-bold uppercase tracking-widest text-white rounded-md">Konversi Nilai</span>
              <a href="#daftar" className="flex items-center justify-center gap-3 rounded-xl sm:rounded-2xl border-4 border-[#191b23] bg-[#9f149f] px-6 py-3 sm:px-8 sm:py-4 text-sm sm:text-lg font-bold text-white shadow-[4px_4px_0_#191b23] sm:shadow-[6px_6px_0_#191b23] transition hover:-translate-x-1 hover:-translate-y-1">MULAI SEKARANG <Icon>trending_flat</Icon></a>
              <a href="#fitur" className="rounded-xl sm:rounded-2xl border-4 border-[#191b23] bg-white px-6 py-3 sm:px-8 sm:py-4 text-sm sm:text-lg font-bold shadow-[4px_4px_0_#191b23] sm:shadow-[6px_6px_0_#191b23] transition hover:-translate-x-1 hover:-translate-y-1">LIHAT DEMO</a>
            </div>
          </div>
        </section>

        <section className="bg-[#f3f3fe] px-5 py-16 sm:py-20 md:px-16">
          <div className="mx-auto max-w-7xl">
            <div className="mb-14 flex items-end gap-8">
              <div>
                <span className="text-xs font-bold uppercase tracking-widest text-[#004ac6]">// IDENTIFIKASI MASALAH</span>
                <h2 className="mt-2 text-2xl sm:text-3xl font-bold uppercase md:text-5xl">Kendala Konversi<br />Nilai Saat Ini</h2>
              </div>
              <div className="mb-4 hidden h-1 flex-grow bg-[#191b23] md:block" />
            </div>
            <div className="grid gap-6 sm:gap-8 md:grid-cols-3">
              {problems.map(([title, text, icon, color]) => (
                <article key={title} className="group rounded-2xl border-4 border-[#191b23] bg-white p-6 sm:p-8 shadow-[6px_6px_0_#191b23] sm:shadow-[8px_8px_0_#191b23] transition hover:-translate-x-2 hover:-translate-y-2 hover:shadow-[10px_10px_0_#004ac6] sm:hover:shadow-[16px_16px_0_#004ac6]">
                  <div className={`mb-6 flex h-16 w-16 items-center justify-center border-[3px] border-[#191b23] transition group-hover:rotate-0 ${color}`}>
                    <Icon className="text-[40px]">{icon}</Icon>
                  </div>
                  <h3 className="mb-4 text-xl sm:text-2xl font-semibold uppercase">{title}</h3>
                  <p className="text-sm sm:text-base leading-relaxed text-[#434655]">{text}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="bg-[#191b23] px-5 py-16 sm:py-20 text-white md:px-16">
          <div className="mx-auto grid max-w-7xl items-center gap-10 lg:gap-16 lg:grid-cols-2">
            <div className="order-2 lg:order-1 px-2 sm:px-4">
              <div className="relative max-w-md mx-auto lg:max-w-none">
                <div className="absolute -left-2 -top-2 sm:-left-4 sm:-top-4 h-full w-full rounded-2xl border-4 border-white bg-[#004ac6]" />
                <img alt="Dashboard visualization" className="relative z-10 w-full rounded-2xl border-4 border-white shadow-[10px_10px_0_rgba(0,74,198,.3)] sm:shadow-[20px_20px_0_rgba(0,74,198,.3)]" src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=900&q=80" />
              </div>
            </div>
            <div className="order-1 flex flex-col gap-5 sm:gap-6 lg:order-2">
              <span className="text-xs font-bold uppercase tracking-widest text-[#a4c9ff]">Solusi Terintegrasi</span>
              <h2 className="text-2xl sm:text-4xl font-bold leading-tight md:text-6xl">Satu Pintu Untuk<br /><span className="text-[#64a8fe]">Masa Depan Akademik.</span></h2>
              <p className="text-sm sm:text-base lg:text-lg leading-relaxed text-[#e1e2ed]">Sistem Informatics mengotomatisasi alur kerja konversi nilai dengan prinsip <strong>Outcome-Based Education (OBE)</strong>. Kami memangkas birokrasi dan menghadirkan transparansi penuh bagi civitas akademika.</p>
              <ul className="mt-2 space-y-3 sm:space-y-4 text-xs sm:text-sm font-bold">
                {['Pemetaan CPL Otomatis', 'Digital Signature Verifikasi', 'Sinkronisasi Data Siakad'].map((item) => (
                  <li key={item} className="flex items-center gap-3 sm:gap-4"><Icon className="text-[#b4c5ff]">check_circle</Icon>{item}</li>
                ))}
              </ul>
            </div>
          </div>
        ...
        <section id="fitur" className="bg-[#faf8ff] px-5 py-16 sm:py-20 md:px-16">
          <div className="mx-auto max-w-7xl">
            <div className="mb-14 text-center">
              <h2 className="text-3xl sm:text-4xl font-bold md:text-6xl">Fitur Utama</h2>
              <div className="mx-auto mt-6 h-2 w-32 border-2 border-[#191b23] bg-[#004ac6]" />
            </div>
            <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
              {features.map(([title, text, icon], index) => (
                <article key={title} className={`flex flex-col rounded-2xl border-4 border-[#191b23] p-6 sm:p-8 shadow-[6px_6px_0_#191b23] sm:shadow-[8px_8px_0_#191b23] transition hover:-translate-y-2 ${index === 2 ? 'bg-[#004ac6] text-white' : 'bg-white'}`}>
                  <span className={`mb-4 text-4xl sm:text-5xl font-bold leading-none ${index === 2 ? 'text-white/20' : 'text-[#004ac6]/20'}`}>0{index + 1}</span>
                  <h3 className="mb-2 text-xl sm:text-2xl font-semibold">{title}</h3>
                  <p className={`flex-grow text-sm sm:text-base leading-relaxed ${index === 2 ? 'text-white/80' : 'text-[#434655]'}`}>{text}</p>
                  <Icon className={`mt-6 text-2xl sm:text-3xl ${index === 2 ? 'text-white' : 'text-[#004ac6]'}`}>{icon}</Icon>
                </article>
              ))}
              <article className="flex flex-col rounded-2xl border-4 border-[#191b23] bg-white p-6 sm:p-8 shadow-[6px_6px_0_#191b23] sm:shadow-[8px_8px_0_#191b23] transition hover:-translate-y-2 lg:col-span-2">
                <span className="mb-4 text-4xl sm:text-5xl font-bold leading-none text-[#004ac6]/20">05</span>
                <div className="flex h-full flex-col items-center gap-6 sm:gap-8 md:flex-row">
                  <div className="flex-grow">
                    <h3 className="mb-2 text-xl sm:text-2xl font-semibold">Monitoring Progres</h3>
                    <p className="text-sm sm:text-base leading-relaxed text-[#434655]">Pantau grafik capaian magang seluruh mahasiswa secara real-time untuk kebutuhan akreditasi program studi.</p>
                  </div>
                  <div className="flex h-24 sm:h-32 w-full items-center border-[3px] border-[#191b23] bg-[#64a8fe] p-4 md:w-48">
                    <div className="h-4 w-full overflow-hidden rounded-full bg-[#191b23]/20">
        ...
        <section id="alur" className="bg-[#e1e2ed] px-5 py-16 sm:py-20 md:px-16">
          <div className="mx-auto max-w-7xl">
            <h2 className="mb-14 text-center text-2xl sm:text-3xl font-bold uppercase italic md:text-5xl">Alur Sistem Konversi</h2>
            <div className="relative flex flex-col items-center justify-between gap-8 md:flex-row">
              <div className="absolute left-0 top-10 hidden w-full border-t-4 border-dashed border-[#191b23] md:block" />
              {steps.map(([title, text, icon], index) => (
                <div key={title} className="relative z-10 flex w-full max-w-[280px] md:w-1/5 md:max-w-none flex-col items-center text-center">
                  <div className={`mb-5 flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-full border-4 border-[#191b23] shadow-[4px_4px_0_#191b23] ${index === 4 ? 'bg-[#2563eb] text-white' : 'bg-white'} transition hover:bg-[#004ac6] hover:text-white`}>
                    <Icon className="text-2xl sm:text-3xl">{icon}</Icon>
                  </div>
                  <div className={`w-full rounded-lg border-[3px] border-[#191b23] p-4 shadow-[4px_4px_0_#191b23] ${index === 4 ? 'bg-[#004ac6] text-white' : 'bg-white'}`}>
                    <h4 className="text-xs font-bold tracking-wide">{title}</h4>
                    <p className="mt-2 text-[11px] sm:text-xs opacity-70 leading-normal">{text}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="daftar" className="purple-grid-bg flex items-center justify-center px-5 py-16 sm:py-24 md:px-16">
          <div className="relative w-full max-w-4xl overflow-hidden rounded-[24px] sm:rounded-[36px] border-[4px] md:border-[6px] border-[#191b23] bg-white p-6 sm:p-10 md:p-20 text-center shadow-[8px_8px_0_#191b23] md:shadow-[16px_16px_0_#191b23]">
            <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full border-4 border-[#191b23] bg-[#64a8fe] opacity-50 hidden sm:block" />
            <div className="relative z-10">
              <h2 className="text-2xl sm:text-4xl md:text-6xl font-bold">Siap Mengelola Konversi Nilai Secara Digital?</h2>
              <p className="mx-auto mb-8 sm:mb-10 mt-4 sm:mt-6 max-w-2xl text-sm sm:text-base lg:text-lg leading-relaxed text-[#434655]">Bergabunglah dengan ratusan mahasiswa AMIKOM lainnya yang telah merasakan kemudahan konversi nilai berbasis OBE.</p>
              <div className="flex flex-col justify-center gap-4 sm:gap-5 sm:flex-row">
                <button type="button" onClick={() => navigatePage('login')} className="rounded-full border-4 border-[#191b23] bg-[#004ac6] px-6 sm:px-10 py-3 sm:py-4 text-base sm:text-lg font-bold text-white shadow-[4px_4px_0_#191b23] sm:shadow-[8px_8px_0_#191b23] transition hover:-translate-x-1 hover:-translate-y-1">DAFTAR SEKARANG</button>
                <button type="button" className="rounded-full border-4 border-[#191b23] bg-[#faf8ff] px-6 sm:px-10 py-3 sm:py-4 text-base sm:text-lg font-bold shadow-[4px_4px_0_#191b23] sm:shadow-[8px_8px_0_#191b23] transition hover:-translate-x-1 hover:-translate-y-1">HUBUNGI KAMI</button>
              </div>
            </div>
          </div>
        </section>
